...
export function EventCard({ event, compact = false }) {
  ...
  return (
    <ParticleCard
      className={`flex flex-col justify-between relative w-full max-w-full p-5 rounded-[20px] border border-[#392e4e] bg-[#060010] font-light overflow-hidden transition-all duration-300 ease-in-out ${compact ? 'event-card-compact' : 'event-card'} card--border-glow`}
      glowColor="132, 0, 255"
      enableTilt={true}
      ...
      {event.img && (
        <div className="w-full flex justify-center ">
          <img
            ...
            style={{
              width: compact ? 200 : 80,
              height: compact ? 200 : 80,
              objectFit: 'cover',
