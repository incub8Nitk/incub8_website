...
export default function DomeGallery({
  ...
  const startAutoRotation = useCallback(() => {
    if (autoRotateRef.current || draggingRef.current || focusedElRef.current || isHoveredRef.current) return;
    
    const rotate = () => {
      if (draggingRef.current || focusedElRef.current || isHoveredRef.current) {
      ...
      onDrag: ({ event, last, velocity = [0, 0], direction = [0, 0], movement }) => {
        ...
        if (last) {
          ...
          if (Math.abs(vx) > 0.005 || Math.abs(vy) > 0.005) {
            ...
            // Restart auto-rotation after a delay if no inertia
            setTimeout(() => {
              if (!draggingRef.current && !focusedElRef.current && !isHoveredRef.current) {
                startAutoRotation();
              }
  ...
  // Mouse enter/leave handlers for auto-rotation control
